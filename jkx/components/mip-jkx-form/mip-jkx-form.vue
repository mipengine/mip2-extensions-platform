<template>
  <div class="form">
      <div class="name flex">
        <p class="left">姓名</p>
        <input type="text" class="right" v-model="form.name" @blur="code" placeholder="请填写您的姓名" :class="{right1:wrong1}">
      </div>
      <div class="phone flex">
        <p class="left">手机号码</p>
        <input type="number" pattern="[0-9]*" class="right" @blur="code" v-model="form.phone" placeholder="请填写您的手机号码" :class="{right1:wrong2}">
      </div>
      <div class="sendCode flex">
         <p class="left">图形验证码</p>
        <input type="number" pattern="[0-9]*" class="right" @blur="code" v-model="form.imgCode" placeholder="请填写图形验证码" :class="{right1:wrong3}">
        <img class="img" :src="form.img.img" @click="refreshImg" />
      </div>
      <div class="code flex">
         <p class="left">短信验证码</p>
        <input type="number" pattern="[0-9]*" class="right" v-model="form.code" @blur="code" @input="inputCode" placeholder="请填写短信验证码" :class="{right1:wrong4}">
...
props:['isForm'],
  ...
  methods:{
    ...
    smscode() {
        ...
        } else if (this.form.imgCode && this.form.imgCode.length != 4 || this.form.imgCode == '') {
            this.imgcode()
            this.form.imgCode = ''
            this.code = ''
            MIP.setData({
