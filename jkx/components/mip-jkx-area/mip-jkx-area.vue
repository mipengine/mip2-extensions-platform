<template>
  <div class="Mask" v-show="areadata.show" ref="mask" @touchmove.prevent>
      <div class="over" :class="{noScroll: isShowPopup}"  >
          ...
          <div class="select" @touchmove.stop>
            ...
            <div class="area1">
              <div class="province">
                <ul>
                  <li v-for="(item,index) in proList" class="pro" @touchmove.stop @click.stop="choosepro(item)" :class="{on:area.proId==item.id}" :key="index">{{item.name}}</li>
                </ul>
              </div>
              <div class="city">
                <div>
                  <div v-for="(item,index) in cityList" class="city_a" @touchmove.stop @click.stop="chooseCity(item)" :class="{cityOn:cityId==item.id}" :key="index">{{item.name}}
                    <ul class="district" v-if="area.cityId==item.id">
                      <li v-for="(item,index) in  districtList" class="city_b" @click.stop="choosedis(item)" :class="{disOn:area.disId==item.id}"  :key="index">{{item.name}}</li>
                    </ul>
                  </div>
                </div>
              </div>
