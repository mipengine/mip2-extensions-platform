    ...
    <div class="hotList">
        ...
        <ul class="list-wrapper flex">
            <li v-for="(item,index) in data" :key="index" class="list" :class="{left:index%2>0}">
                <a :href="`https://openapi.baidu.com/oauth/2.0/authorize?response_type=code&client_id=dsAkhUrPV9EGA2aId4iv8E9ob8cam6bG&redirect_uri=https://t.jikexiu.com/resources/mip/order.html?modelName=${item.modelName.split(' ').join('')},${item.deviceId},${item.malfunctionName},${item.price},${item.malfunctionId},${item.deviceId}&scope=snsapi_userinfo&state=STATE`">
