<template>
  <div v-show="devicedata.show" class="mask" ref="mask" @touchmove.prevent> 
...
export default {
  ...
  watch:{
    devicedata(val){
      if(val.last){
        this.last = val.last
        this.tab = ['类型','品牌','型号']
        this.changeColor=val.changeColor
        this.queryBrand()
      }else{
        this.last = val.last
        this.tab = ['分类','故障']
        this.changeColor1=val.changeColor1
        this.queryBrand()
      }
    }
  },
  ...
  created(){
    ...
    this.queryBrand()
    // else if(!this.devicedata.last&&this.devicedata.name&&this.devicedata.name1&&this.devicedata.price>0){
    //   console.log('hbb')
    //     this.last =this.devicedata.last
    //     this.tab = ['分类','故障']
