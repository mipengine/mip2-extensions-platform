<template>
    <div class="comp-home-header-container">
      <a class="top-search" data-type="mip" :href="[baseUrl + '/product/search']">
        <span class="iconfont icon-sousuo"></span>
        <span class="text">点击搜索您要评估的型号</span>
      </a>
      <template v-if="bannerLength">
        <mip-carousel autoplay layout="responsive" width="750" height="360">
          <mip-img v-for="img in bannerList" :key="img.id" layout="responsive" :src="img.file_url"></mip-img>
        </mip-carousel>
      </template>
    </div>
</template>
