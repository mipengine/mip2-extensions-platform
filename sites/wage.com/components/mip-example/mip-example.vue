<template>
  <div id="Container">
    <div v-show="resultShow" class="pt55">
      <mip-fixed type="top" class="tag-nav-top">
        <div v-show="resultShow" class="tag-nav">
          <div class="flex-item">
            <span :class="{active:active}" @click="selectTag(1,active)">计算税后工资</span>
            <span :class="{active_select:active}" />
          </div>
          <div class="flex-item">
            <span :class="{active:!active}" @click="selectTag(2,!active)">推算税前工资</span>
            <span :class="{active_select:!active}" />
          </div>
        </div>
      </mip-fixed>
      ...
      <div :class="{box:isOk}">
        ...
        <mip-social-security :fromwage="wage" :tag="tag" :isclick="isClick" :socialsecurity="socialSecurity" @securityselect="securitySelect"
          @getsecuritybase="getSecurityBase" @getselectvalue="getSelectValue" @islowst="isLowst">是否缴纳社保</mip-social-security>
        <mip-social-security :isshow="show" :fromwage="wage" :tag="tag" :isclick="isClick" @securityselect="securitySelect" @getsecuritybase="getSecurityBase"
          @getproportion="getProportion" @getselectvalue="getSelectValue" @islowst="isLowst">是否缴纳公积金</mip-social-security>
      </div>
      <div id="calculateBtn" class="calculateBtn" @click="calculate">开始计算</div>
      <!--提示框-->
      <mip-fixed :class="{none:tipShow}" type="top" class="container">
        <div id="tips" :class="{none:tipShow}" class="tipsMes">{{ tipsmes }}</div>
      </mip-fixed>
    </div>
    <mip-result v-show="!resultShow" :resultdata="resultData" />
    <div id="tips" :class="getPositionValue" class="tips">企微云提供技术支持</div>
  </div>
</template>
<style scoped>
  .pt55 {
