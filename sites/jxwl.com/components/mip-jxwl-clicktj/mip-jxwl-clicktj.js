export default class MIPJxwlClickTj extends MIP.CustomElement {
  build () {
    let req = new Request('https://api.youxigt.com/ajax/hits/type/click', {
      'mode': 'no-cors'
    })
    ...
    az.addEventListener('click', e => {
      ...
      fetch('https://api.youxigt.com/ajax/hits/type/down?url=' + aurl).then(function () {})
    })
    ios.addEventListener('click', e => {
      let iurl = ios.getAttribute('href')
      fetch('https://api.youxigt.com/ajax/hits/type/down?url=' + iurl).then(function () {})
    })
  }
}
