    ...
    if (document.getElementsByClassName('hot_words').length) {
      fetch(`${this.data().url}/ajax.asp?action=894&position=2`, {
        ...
        for (var i = 0; i < list.length; ++i) {
          list_html += '<a href="/s/' + list[i].app_id + '" data-stats-cnzz-obj="{\'type\':\'click\',\'data\':[\'_trackEvent\',\'reci\',\'reci' + (i + 1) + '\',\'' + list[i].title + '\']}">' + list[i].title + '</a>';
        }
        document.getElementsByClassName('hot_words')[0].getElementsByClassName('dcatetory')[0].innerHTML = list_html       
      })
    }
  }
  /*
