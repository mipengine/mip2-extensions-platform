...
export default class MIPWstimesView extends MIP.CustomElement {
  build() {
    ...
    if (type === 'text') {
      ...
      content.innerHTML = '<a target="_blank" href="' + data.href + '" class="adv-text dan">' + data.text + '</a>'
      this.element.appendChild(content)
    } else if (type === 'image') {
      content.className = 'view-img'
      content.innerHTML = '<a target="_blank" href="' + data.href + '"><mip-img class="img" src="' + data.src + '" style="width:' + (size[0] / 375) * 100 + 'vw;height:' + (size[1] === 'auto' ? 'auto' : (size[1] / 375) * 100 + 'vw') + '"></mip-img></a>'
      this.element.appendChild(content)
    } else if (type === 'graphic') {
      content.className = 'view-graphic'
      content.innerHTML = '<a target="_blank" href="' + data.href + '" class="item"><div class="info"><h3 class="info-title shuang">' + data.title + '</h3><div class="info-about"><span>' + data.date + '</span><span>阅读(' + data.total + '+)</span></div></div><mip-img class="img" src="' + data.src + '"></mip-img></a>'
      this.element.appendChild(content)
      ...
      for (let i = 0; i < data.list.length; i++) {
        swiperItems += '<a target="_blank"  href="' + data.list[i].href + '"><mip-img class="img" src="' + data.list[i].src + '"></mip-img></a>'
        if (i === 0) {
