<template>
  <section main="role">    
    <div v-if="orderList.length > 0">
      <div class="order-item" v-for="order in orderList" v-bind:key="order.Id">
        <div class="order-item-wrap" v-on:click="orderDetail(order)">
          <div class="order-item-hd">
            <div v-bind:class="{ gray: order.Status === 10132005, orange: order.Status !== 10132005 }">{{order.OrderStatusName}}</div>
          </div>
          ...
          <div class="ui-flex align-center">
            <div class="cell order-item-bd">
              ...
              <div class="ui-flex algin-start">
                <div>车型：</div>
                <div class="cell">{{order.CarType.Name}}/{{order.CarType.GearName}}/{{order.CarType.StructureName}}/{{order.CarType.Emission}}{{order.CarType.EmissionUnit}}</div>
              </div>
              <div class="ui-flex algin-start">
                <div>取车门店：</div>
                <div class="cell">
              ...
              <div class="ui-flex algin-start">
                <div>总价：</div>
                <div class="cell orange"><text class="rmb">¥</text>{{order.TotalAmount}}</div>
              </div>
            </div>
