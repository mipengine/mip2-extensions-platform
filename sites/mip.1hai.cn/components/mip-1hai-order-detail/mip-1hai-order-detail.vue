<template>
  <section main="role">
    <div class="mod order-detail-order">
      ...
      <div>
        <span class="gray">支付状态：</span>
        <span class="orange">{{ orderDetail.OrderPayStaus }}</span>
      </div>
    </div>
    ...
    <div class="mod order-detail-info">
      <div class="ui-flex align-center info-car">
        <div class="item-pic">
          <mip-img
            :data-src="orderDetail.ImagePath"
        ...
        <div class="item-title">
          ...
          <div
            class="gray"
          >{{ orderDetail.GearName }} | {{ orderDetail.EmissionUnit }} | {{ orderDetail.StructureName }} | {{ orderDetail.MaxPassenger }}</div>
        </div>
      </div>
      ...
      <div class="info-store">
        ...
        <div
          v-if="orderDetail.ActualDropoffTime"
          class="gray info-store-time">实际用车时间： {{ orderDetail.ActualPickupTime }} 至 {{ orderDetail.ActualDropoffTime }}</div>
      </div>
    </div>
    ...
    <div class="mod order-detail-price">
      ...
      <div
        v-for="item in orderDetail.PriceDetails"
        :key="item.Type"
        class="price-detail">
        <div class="ui-flex align-center">
          <div class="name">{{ item.PriceTitle }}</div>
          <div class="dotline cell"/>
          <div
            v-if="item.Type != 1"
            class="money">{{ item.PriceTotal }}元</div>
          <div
            v-if="item.Type == 1"
            class="money green">-{{ item.PriceTotal }}元</div>
        </div>
      </div>

      <div
        v-if="orderDetail.GuaranteeFee"
        class="price-total">
        <div class="ui-flex align-center price-total-bd">
          <div class="cell">预授权
            <span>{{ orderDetail.GuaranteeFee }}</span>元
          </div>
          <div class="money">总价
            <span class="orange">{{ orderDetail.AmountTotal }}</span>元
          </div>
        </div>

        <div
          v-if="orderDetail.IsShowGuaranteeTip"
          class="orange"
          style="font-size: .857em; padding-bottom: 10rpx;"
        >{{ orderDetail.GuaranteeTip }}</div>

        <div class="ui-flex align-center price-total-bd">
          <div class="cell">违章押金
            <span>{{ orderDetail.IllegalFee }}</span>元
          </div>
        </div>
      </div>
    ...
    <div
      v-if="orderDetail.PaidAmount > 0 && orderDetail.AmountTotal > orderDetail.PaidAmount"
      class="mod panel order-detail-price-status"
    >
      <div class="item">
        <div class="item-bd">已付金额：
          <span class="orange">{{ orderDetail.PaidAmount }}</span> 元，还需支付
          <span class="orange">{{ orderDetail.PaymentAmount }}</span>元
        </div>
      </div>
    </div>

    <div
      v-if="orderDetail.PaymentAmount <= 0"
      class="mod panel order-detail-price-status">
      <div class="item">
        <div class="item-bd">已付金额：
          <span class="orange">{{ orderDetail.PaidAmount }}</span>元
        </div>
    ...
    <div
      v-if="orderDetail.IsAllowCancel"
      class="mod ui-flex align-center order-detail-operate">
      <!-- <div class="cell ui-btn btn-inline btn-lg btn-primary-line">修改</div> -->
      <div
        class="cell ui-btn btn-inline btn-lg btn-primary-line"
        @click="cancelOrderWrap">取消</div>
    </div>
    ...
    <!-- <div class="pay-total" v-if="orderDetail.Status == '10132001' && orderDetail.OrderType != '10118007' && orderDetail.PaymentAmount > 0">
      <div class="money">
        ...
        <div>订单总额：<span class="rmb">¥</span><span>{{ orderDetail.PaymentAmount }}</span/>
      </div>
    </div>-->
    <div
