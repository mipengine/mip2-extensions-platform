  ...
  <section main="role">
    <div class="home-head">
      <div class="bd text-center">
        <div class="face bg-avatar"></div>
        <div class="user-rating">
          <div class="ui-flex-colum-wrap align-center justify-center">
            <div class="cell bg-heart" v-bind:class="['bg-heart-' + user.SelfLevel]"></div>
    ...
    <div class="mod mgt0 home-mod">
      ...
      <div class="ui-flex text-center line-separate">
        <div class="cell">
          <div class="item-hd orange"><span>{{user.Balance || 0}}</span>元</div>
          <div class="item-bd gray">用户余额</div>
        </div>
        <div class="cell">
          <div class="item-hd orange"><span>{{user.UserPoint || 0}}</span>分</div>
          <div class="item-bd gray">用户积分</div>
        </div>
        <div class="cell">
          <div class="item-hd orange"><span>{{user.FreeDays || 0}}</span>天</div>
    ...
    <div class="mod panel item-list">
      <div class="item border-top-px1" @click="wallet('order')">
        <span class="icon-m icon-order"></span>
        <p class="item-bd">订单管理</p>
        <span class="arrow aw-right"></span>
      </div>
    </div>

    <div class="btn-lg-wrap">
      <a data-type="mip" data-title="登录" href="login.html" class="ui-btn btn-lg btn-block btn-primary2" v-if="!hasLogin">登录</a>
      <div class="ui-btn btn-lg btn-block btn-primary2" v-if="hasLogin" @click="loginOut">退出登录</div>
    </div>
