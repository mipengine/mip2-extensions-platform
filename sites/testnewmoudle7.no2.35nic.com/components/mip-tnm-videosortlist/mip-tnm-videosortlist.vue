  ...
  <div class="lists videos">
    <a
      ...
      <li
        ...
        :key="noitemli.videoId">
        <a
          :href="noitemli.thePageUrl"
          class="itemlink">
          <div class="img">
            <mip-img
              :src="noitemli.videoImage"
              layout="responsive"
              width="100"
              height="100"/>
          </div>
          <div class="topic">{{ noitemli.videoTopic }}</div>
        </a>
      </li>
    </a>
    <a
      ...
      <li
        ...
        <a
          ...
          <div class="img">
            <mip-img
              :src="itemli.videoImage"
              layout="responsive"
              width="100"
              height="100"/>
          </div>
          <div class="topic">{{ itemli.videoTopic }}</div>
        </a>
      </li>
  ...
  mounted () {
    fetch(this.sortsrc).then(response => {
      if (response.ok) {
        ...
        throw new Error('videos sort info went wrong!')
      }
    }).then(sortinfo => {
      let self = this
        ...
        (function (i) {
          ...
          fetch(that.src + '0_' + that.sort_list[i].mnSortId).then(response => {
            if (response.ok) {
              ...
              throw new Error('videos sort list went wrong!')
            }
          }).then(iteminfo => {
            let sobj = {}
      ...
      fetch(this.src + '0').then(response => {
        ...
        nosobj.sortData = noprosort.data.items.filter((istopnosortitem) => {
          return istopnosortitem.videoIsTop === 1
        })
