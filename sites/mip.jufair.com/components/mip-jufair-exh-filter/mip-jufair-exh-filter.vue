...
export default {
  ...
  mounted () {
    this.clientHeight = window.screen.height - (0.93333333 * window.screen.width / 10) + 'px'
  },
  methods: {
    Url (url, pid, id, type) {
      url = url.replace(/\//g, '')
      ...
      // 切換條件的時候，讓頁面置為1
      temp[7] = 1
      if (type === 'firstIndustryName') {
        temp[1] = id
        temp[2] = 0
      } else if (type === 'secondIndustryName') {
        temp[1] = pid
        temp[2] = id
      } else if (type === 'continentName') {
        temp[3] = id
        temp[4] = 0
        temp[5] = 0
