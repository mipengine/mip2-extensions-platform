    ...
    <ul v-show="!isEmpty">
      <li v-for="(item,index) in articlesList" :key="index" @click="goArticleDetail(item.articlesId)">
        <div class="leftContent">
          ...
          <p>
            <mip-img src="http://haya-cloud.oss-cn-shanghai.aliyuncs.com/haya-cloud/1565176807216eye.png" class="eye"></mip-img>
            <span>{{item.articlesReadNums}}</span>
          </p>
        </div>
        <div class="rightContent">
          <mip-img :src="item.articlesImg" layout="fill" class="articleImg"></mip-img>
        </div>
      </li>
    </ul>
      ...
      .then(function(data) {
        ...
        if (data.resultCode == 10000) {
          if(data.result.articlesList != null){
            that.articlesList = that.articlesList.concat(data.result.articlesList);
