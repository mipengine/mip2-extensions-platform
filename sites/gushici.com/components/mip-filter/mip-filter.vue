  ...
  <div class="panel filter-panel"> 
    ...
    <div class="filter-body"> 
      <div class="filter-item"
        ...
        <div v-if="item.show" class="item-handler" @click="_open" :data-i="i"></div>
      </div> 
    </div> 
  </div>
</template>
...
export default{
  ...
  methods:{
    param(){
      ...
      // 测试
      path = 'd_2.html'.replace('.html','').split('_')
      let q = {}
      if(path.length>2){
    ...
    */
    init(){
      ...
      let arr=[]
      let t = this
      let key_values = this.key_value
      for(let i=0;i<key_values.length;i++){
        for(let j=0;j<keys.length;j++){
          if(keys[j]==key_values[i].cn){
            arr.push(key_values[i])
          }
        }
      }
    ...
    _changefilter(q){
      ...
      t.cur = q.val
      let old={},old_i=null,old_ci=null
      ...
      if(old_i!=null&&old_ci!=null){
        t.list[old_i].arr.splice(old_ci,1)
        old.iscur=true
        t.list[old_i].arr.unshift(old)
      }
