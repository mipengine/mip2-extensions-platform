...
export default class MIPDataUrl extends MIP.CustomElement {
  ...
  attributeChangedCallback(page, old, cur){
    ...
    console.log('old page',old)
    console.log('cur page',cur)
  }
  ...
  _so(res,value){
    let poems = [],auth={},haspoem=false,hasauth=false,has=false
    ...
    for(let i=0;i<res.poetry.length;i++){
      ...
      let item={
        ...
        has_y:info.translation==''?false:true,
        has_z:info.explanation==''?false:true,
        has_s:info.appreciation==''?false:true,
        line:o.line,
        s:o.s,
        t:o.t,
        is_y:false,
  ...
  _auth(res){
    let info = res.detail
    let tags=info.poe_list,tags_count=info.num
    let tabs = [
      {"iscur":true,"htm":""},
    ...
    MIP.setData({auth,head,tabs,tags,tags_count})
  }
  ...
  _poem(res){
    ...
    for(let i=0;i<res.like.length;i++){
      ...
      let lk = {
        ...
        has_y:res.like[i].translation==''?false:true,
        has_z:res.like[i].explanation==''?false:true,
        has_s:res.like[i].appreciation==''?false:true,
        line:lo.line,
        s:o.s,
        t:o.t,
        is_y:false,
        is_z:false,
        is_s:false,
        hasmore:lo.line_count>7?true:false
      }
      likes.push(lk)
    }
