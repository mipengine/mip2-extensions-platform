...
export default class MIPJtStockTool extends MIP.CustomElement {
    build() {
        ...
        function alert(msg) {
            ...
            setTimeout('document.getElementById("tips").style.display = "none";document.body.classList.remove("noscroll")', 1500);
        }
     ...
     */
        ...
        function calcRatio() {
            ...
            var valprice = parseFloat($('edprice').value);
            var valone = parseFloat($('edone').value);

            var valresult = round(valprice / valone);
     ...
     */
        function calcTradeFee() {
            ...
            var obj = ComputeTradeFee(parseInt($('TradeType').value));
        ...
        function ComputeTradeFee(iType) {
            var tradenum = parseInt($('edtradenum').value);
            var tradeprice = parseFloat($('edtradeprice').value);
            var traderate = parseFloat($('edtraderate').value) / 100;
        ...
        function checkPN(CheckCtl, disptext, IsCanZero, page) {
            var s = trim(CheckCtl.value).toString();
        ...
        function checkFn(CheckCtl, disptext, page, floatcount) {
            ...
            if ((isNaN(temp)) || (temp < 0) || (temp !== parseFloat(s))) {
                ...
                var limitcount = floatcount ? floatcount: 2;
                var array = s.split('.');
                var count;
        ...
        function changeRateStock() {
            var valueBox = document.getElementById('stock_stockType').value;
        ...
        function execStock() {
            ...
            var tradeSum = document.getElementById('stock_buyTradeSum').value;
            var buyTax = document.getElementById('stock_buyTax').value;
            document.getElementById('stock_buyCost').value = formatFloat(Number(tradeSum) + Number(buyTax), 2);
            ...
            var sellTradeSum = document.getElementById('stock_sellTradeSum').value;
            var sellTax = document.getElementById('stock_sellTax').value;
            document.getElementById('stock_gainSum').value = formatFloat(Number(sellTradeSum) - Number(sellTax), 2);
