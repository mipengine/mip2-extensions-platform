...
export default class MIPTbsalert extends MIP.CustomElement {
    build () {
        ...
        function tipsMess(type, content) {
            switch (type) {
                ...
                case 'checking':
                    ...
                    // 提示弹窗
                    let tipsWindows = `<div class="tips-content">${ !!content ? content : '未知的操作'}</div>`;
        ...
        // 根据省份获取城市信息
        function setProvicegetCity(cityDatas, proviceId) {
            let source = cityDatas ? cityDatas : allCity;
            let list = [];
            source.find(function (element) {
        ...
        if (document.querySelector('.custom-province-city')) {
            ...
            provincecity.onclick = function () {
                ...
                for ( let i =0 ; i < areaSelect.length; i++) {
                    ...
                    areaSelect[i].addEventListener('touchend', function (e) {
                        ...
                        if (li.parentElement.classList.contains('area-select-province')) {
                            ...
                            li.classList.add('on');
                            // 重新获取城市 province_id
                            let newList = setProvicegetCity(null, currentProvince);
                            // 根据选择渲染城市信息
                            let newCityHtml = '';
                            for (let j = 0; j < newList.length; j++) {
                                ...
                                newCityHtml += '<li value="' + cityId + '" data-citygrade="' + cityGrade + '" >'
                                            + fullName + '</li>';
                            };
                ...
                document.querySelector('.m-area-select-background') && 
                document.querySelector('.m-area-select-background').addEventListener('click', function () {
        ...
        // 关闭整个弹窗
        document.querySelector('.top-close') && 
