<template>
    <div id="addressSelect" v-show="show" ref="wrapper">
        <div class="jkx_item">
            <div class="item_right pl_20">
                <div class="jkx_form delete">
                    <i class="search"></i>
                    <input type="text" placeholder="请输入具体位置" v-model="inputVal" @keyup="searchVal()" />
                    <em class="jkx_m_icon deleteIcon" v-if="this.inputVal!=''" @click="empty()"></em>
                </div>
            </div>
        </div>
            ...
            <section class="jkx_content">
                <div class="jkx_cell position">
                    <span class="cell_title gray1 currentLocation" @click="Choose(choosedetail.nowPosition)">{{choosedetail.nowPosition}}</span>
                    <span class="cell_info">
                        <i class="jkx_m_icon"></i>
                        <em class="orange fs_24">重新定位</em>
                    </span>
                </div>
            </section>
            <section class="jkx_container">
                <h3 class="jkx_title nearAddress" id="panel">附近地址</h3>
            </section>
            <section class="jkx_content nearAddressList">
                <div>
                    <div v-for="(item,index) in html" :key="index">

                        <div class="jkx_panel" @click="Choose(item.title||item.name)">
                            <p class="panel_title">{{item.title||item.name}}</p>
                            <p class="panel_body">{{item.address}}|({{item.province}} {{item.city}} {{item.district}})</p>
                        </div>
                    </div>
                </div>
                <span class="jkx_loading"></span>
            </section>
    </div>
</template>
...
input{
    ...
    .back {
        /* background-image: url(../../../src/assets/images/icon_back2.png); */
        width: 30px;
        height: 30px;
    ...
    .jkx_form.delete .deleteIcon {
        ...
        right: 10px;
        margin-right: 0;
        /* background-image: url(../../../src/assets/images/icon_del.png); */
         width: 30px;
        height: 30px;
        background-size: 100% 100%;
        display: inline-block;
