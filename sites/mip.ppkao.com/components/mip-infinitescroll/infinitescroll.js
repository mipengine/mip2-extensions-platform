...
export default class MIPInfinitescroll extends MIP.CustomElement {
  build () {
    let $ = require('jquery')
    ...
    InfiniteScroll.prototype = {
      ...
      _bindScroll: function () {
        ...
        me.options.$wrapper.on('scroll' + me.eventSpace, function () {
          ...
          // 获取当前可视区页码
          let currentShowPage = me.getShowPage()
          // 若页码变化
      ...
      _scrollBottomFn: function () {
        ...
        let pn = me.currentLoadPage + 1 // 需要加载的页码(从0计)
        let dn = me.scrollPageCache.content.length - 1 // 已有数据最大页码(从0计)
        ...
        if (me.dataStatus === 1 && pn + me.options.preLoadPn >= dn) {
          ...
          let dataDeferred = me.options.pushResult.call(me, (dn + 1) * me.options.pageResultNum, dn - pn)
          // 标记数据状态为请求中
          me.dataStatus = 2
      ...
      _updateScrollElement: function (pn) {
        ...
        let $domNewPage = $(me._wrapPageParentDom(me.scrollPageCache.content[pn], pn))
      ...
      _cycleScrollElement: function (pn) {
        ...
        let startPage = Math.max(pn - Math.floor((me.options.limitShowPn - 1) / 2), 0)
        // 获取所有结果列表dom
        let $domResultElement = me.options.$result.find('.' + me.options.scrollPageClass)
        // 选出当前需要被显示的dom页
        let $domShouldShowElement = $domResultElement.slice(startPage, startPage + me.options.limitShowPn)
      ...
      _separatePage: function (listArr) {
        ...
        let pageResultNum = me.options.pageResultNum // 每页结果数
        let pageNum = Math.ceil(listArr.length / pageResultNum) // 分成x页
        let pageHtmlArr = []
      ...
      /**
       * 为每页内容包裹父容器
       * IP:html-[string]一页的html代码片段pn-[number]当前页码
       ...
       * 获取当前可视区页码的方法(从0计)
       *
       * @returns {integer} 0 或 1
       */
      getShowPage: function () {
        let me = this

        let scrollPageCacheTopPosition = me.scrollPageCache.topPosition.concat()
