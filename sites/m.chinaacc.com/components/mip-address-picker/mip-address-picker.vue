<template>
  <div class="distpicker-address-wrapper" v-if="show">
    <div class="adressMaker" v-if="show"></div>
    <div class="aressWrapper">
      <div class="address-header">
        <ul class="clearfix">
          <li class="fl" :class="{'active' : tab === 1}" @click="resetProvince">
            <span>{{currentProvince.name ? currentProvince.name : placeholders.province}}</span>
          </li>
          <li class="fl" v-if="showCityTab" :class="{'active' : tab === 2}" @click="resetCity">
            <span>{{currentCity.name ? currentCity.name : placeholders.city}}</span>
          </li>
          <li class="fl" v-if="showAreaTab" :class="{'active' : tab === 3}">
      ...
      <div class="address-container">
        <ul v-if="tab === 1">
          <li v-for="(item, index) in provinces" :key="index" @click="chooseProvince(item.areaID,item.areaName)">{{item.areaName}}</li>
        </ul>
        <ul v-if="tab === 2">
          <li v-for="(item, index) in citys" :key="index" @click="chooseCity(item.areaID,item.areaName)">{{item.areaName}}</li>
        </ul>
        <ul v-if="tab === 3">
          <li v-for="(item, index) in areas" :key="index" @click="chooseArea(item.areaID,item.areaName)">{{item.areaName}}</li>
        </ul>
      </div>
    </div>
  </div>
