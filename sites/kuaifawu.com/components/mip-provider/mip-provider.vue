        ...
        <div class="item">
          <a :href="geturl('/product/info.html?id=' + pid + '_' + ct + '_' + ar + '_' + val.id +'_' + packageid)">
            ...
            <div class="right">
              ...
              <p class="r-info">{{ val.areatitle }}<span class="line"/>{{ val.workyearnum }}年工作经验<span class="line"/>{{ val.company }}</p>
              <p class="rate">最近三个月接咨询量：<span>{{ val.recentordersnum }}</span></p>
            </div>
          </a>
        </div>
      </div>
...
<style scoped>
  ...
  .page .item a .left mip-img{width:100%;height:100%;border-radius: 50%;}
  .page .item a .left mip-img img{width:100%;height:100%;border-radius: 50%;}
  .page .item a .right{font-size: 0.16rem;color: #666;width:9rem;word-wrap: break-word;}
  .page .item a .right .name{color: #333;font-size: 0.18rem;font-weight: bold;line-height: 0.4rem;}
  .page .item a .right .r-info{font-size: 0.14rem;line-height: 0.2rem;}
  .page .item a .right .rate{font-size: 0.14rem;line-height: 0.4rem;}
  .page .item a .right .rate span{display: inline-block;color: #e85555;}
  .line{width:0.01rem;height: 0.14rem;margin:0 0.1rem;background: #d9d9d9;display: inline-block;vertical-align: middle;}
</style>
