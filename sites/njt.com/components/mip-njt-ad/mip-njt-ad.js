    ...
    function loadXml () {
      ...
      fetch('/API/pageajax.ashx', {
        ...
        res.json().then(function (data) {
          ...
          if (isLbs) {
            fetch('/API/IP.ashx?action=json', {
              method: 'GET'
            }).then(function (ipres) {
              ipres.json().then(function (remoteIpInfo) {
                if (remoteIpInfo) {
                  if (remoteIpInfo.ret === 1) {
                    for (let q in adplace) {
                      loadadm(adlist, remoteIpInfo.province, remoteIpInfo.city, adplace[q])
                    }
                  } else {
