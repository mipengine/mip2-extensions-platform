...
export default class MIPNjtAd extends MIP.CustomElement {
   ...
   * 构造元素，只会运行一次
   */
    var element = document.querySelectorAll("mip-njt-ad")[0];
    ...
    function loadadm(admxml, province, city, placeName) {
        $(admxml).find('adplace').each(function () {
            if ($(this).find('placeName').text() === placeName) {
                $(this).find('item').each(function () {
                    var flag1 = $(this).find('pagename').text() === ''|| $(this).find('pagename').text().split(",").indexOf(pagename) >= 0;
                    var flag2 = $(this).find('typeid').text() === ''|| $(this).find('typeid').text().split(",").indexOf(ptypeid) >= 0;
                    var flag3 = $(this).find('categoryid').text() === ''|| $(this).find('categoryid').text().split(",").indexOf(pcategoryid) >= 0;
                    var flag4 = $(this).find('province').text() === ''|| $(this).find('province').text().indexOf(province) >= 0|| province.indexOf($(this).find('province').text()) >= 0;
                    var flag5 = $(this).find('city').text() === ''|| $(this).find('city').text().indexOf(city) >= 0|| city.indexOf($(this).find('city').text()) >= 0;
