<template>
  <div class="wrapper">
     <mip-fixed type="top" class="listWrap" v-show="isFixed">
         <ul class="titleListWrap" ref="scrollX">
          <li class="titleLi" :class="[currentIndex == index ?'currentTitle':'']" v-for="(item,index) in report_title" 
          :key="index"
          @click="jump(index)"
          >{{item.cat_name}}</li>
      </ul>
      ...
      <div v-if="isShowVideo">
          ...
          <!-- <div class="videoMask"  v-if="!isShowVideo && videoImg">
              <img class="maskImg" :src="videoImg" alt="">
              <div class="maskContent">
                <span>优信{{repairSum}}项车况检测，播放时长{{videoTime}}分钟</span>
                <div class="playButon" @click="playing">开始播放</div>
              </div>
          </div> -->
          <mip-car-video  id="test" :video-img="videoImg" :video-url="videoUrl" v-if="videoUrl&&videoImg"></mip-car-video>
      </div>
      <div class="reportTitleWrap"  v-for="(item,index) in car_repair_num" :key="index">
          <div class="boxTop"></div>
          <div class="reportTitle" >
            <span class="title">{{item.cat_name}}</span>
          </div>
          <div class="checkContent"  v-for="(checkItem,index) in (item.child)" :key="index">
              <div class="checkTitleWrap" @click="switchCheckDesc(checkItem.cat_name)">
                 <div class="checkTitle">
                    <span>{{checkItem.cat_name}}</span>
                    <img :class="[carTypeStatus[checkItem.cat_name]?'angleDir':'']" src="http://c2.xinstatic.com/f3/20180416/1840/5ad47da2428ad535567.png"/>
                  </div>
                  <div class="checkoutCountWrap">
                        <div class="checkCount" v-if="checkItem.flaw_all_num">
              ...
              <div class="checkDescWrap" v-show="carTypeStatus[checkItem.cat_name]"  v-for="(flawItem,index) in (checkItem.child)" :key="index">
                 <div class="checkDesc">
                    <span class="descText">{{flawItem.cat_name}}</span>
                    <span>
                        <img v-if="flawItem.flaw_num"  class="reportIcon" src='http://c2.xinstatic.com/f3/20180416/1840/5ad47da23e611535388.png'/>
                        <img  v-else class="reportIcon" src='http://c2.xinstatic.com/f3/20180416/1840/5ad47da23e2e4430972.png'/>
                    </span>
                 </div>
                 <div class="flawDescWrap" v-if="flawItem.flaw_code_describe">
      ...
      <mip-car-button  :source="4" v-if="carid"></mip-car-button>
      <div class="boxTop"></div>
          <div class="reportTitle lastTitle">
          <span class="title">瑕疵及修复</span>
      </div>
      <mip-flaw-switch :carid="carid" v-if="carid" class="borderBottom"></mip-flaw-switch>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    requestFun("/ajax/report/car_report", {
      ...
      if(res.report_complete_video.video_url && res.report_videos_all){
          let report_complete_video = res.report_complete_video;
          let report_videos_all = res.report_videos_all;
          let video = report_videos_all.biao
            ? report_videos_all.biao
            : report_complete_video; //无分级视频时，使用完整视频
          this.videoUrl = video.video_url; 
          this.videoImg = report_complete_video.pic;
  ...
  methods: {
    switchCheckDesc: function(carname) {
      ...
      carTypeStatus[carname] = carTypeStatus[carname] ? false : true;
    ...
    jump(index) {
      ...
      this.flag = false;
      let jump = document.querySelectorAll(".boxTop");
      let total = jump[index].offsetTop - 40; //mip的mip-shell的高度会影响页面的滚动，所以将其从滚动的距离中减去
