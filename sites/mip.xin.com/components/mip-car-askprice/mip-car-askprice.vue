<template>
  <div v-if="showToast" id="askPrice" :class="{'mask-top':showSimilarToast && showList}">
    <!-- 询价弹框 -->
    <mip-fixed type="top" id="callmask" @click="hideMask"  v-if="showToast && showAskToast" class="callMask">
        <div class="enquiryContent" id="askPrice">
          <span class="leaveContact">提交询价，立即获取最低报价</span>
          <span class="car-price-current">本车可议价，当前报价{{carPrice}}万元</span>
          <div class="input-box">
            <mip-form>
            <input class="enquiryTel" id='telInput' 
              type="tel" 
              validatetype="phone" 
              maxlength="11" 
              @input="enquiryInput" 
            ...
            <mip-img class="close" src='http://s4.xinstatic.com/m/img/smallprogram/close.png' @click="clearTel" />

          </div>
          
            <div v-if="remain" class="remain-correct-tel">{{reminMessage}}</div>
          <div class="tel-submit-query">
            <div class="bd-auth" v-if="!info.isLogin"  @click="authMessage" >
            
              <!-- <input type="checkbox" :value="baAuth" checkedid="bdAuth">  -->
              <mip-img class="auth-icon " src="http://c2.xinstatic.com/f3/20180803/1517/5b64017eea4a3722229.png" v-if="baAuth"></mip-img>
              <mip-img class="auth-icon " src="http://c2.xinstatic.com/f3/20180803/1517/5b6401764c164734049.png" v-if="!baAuth"></mip-img>
              <label for="bdAuth" class="auth-font">登陆百度账号，第一时间了解订单状态</label>
            </div>
           
            <button :style="{'margin-top':authBottom}" v-if="!info.isLogin && baAuth && telCorrect"  class="enquiryBtn tel-submit"   on="tap:customlog.login">获取底价</button>
             <button :style="{'margin-top':authBottom}" v-else  class="enquiryBtn tel-submit"  @click='queryClick'>获取底价</button>
          </div>
          
        </div>
    </mip-fixed>
    <!-- 相似推荐弹框 -->
    <mip-fixed type="top" id="callmaskSimilar" v-if="showToast && showSimilarToast"  @click="hideMask"  class="callMask">
       <div class="askprice-success-box" id="similarToast">
        <div class="ask-price-success">
           <mip-img class="askprice-success-boximg" src="http://c2.xinstatic.com/f3/20180717/1102/5b4d5c379c451177647.png"/>
           <span class="askprice-success-font">询价成功</span>
        </div>
        <!-- <span class="askprice-focus-xiongzhang">点击关注优信二手车熊掌号，获取最新二手车动态</span>
        <span class="askprice-focus">
          <strong>+</strong>
           关注
        </span> -->
        
      </div>
      <div style="border-top: 0.12rem solid rgba(246, 246, 249, 1);" v-if="showList && carList.length > 0">
        ...
        <div style="max-height: 5rem;overflow: scroll;"> 
            <div class="similar-recommend" v-for="(item, index) in carList" :key="index">
            <div class="carcontent" :class="{'carcontent-bottom': index != carList.length-1}">
              <mip-img class="contentleft" :src="picHead + item.pic" />
              <div class="contentright">
                <div class="righttop">
                  ...
                  <div class="pricedetail">
                    <span class="similar-recommend-price">{{item.panel_price}}万元</span>
                    <span class="similar-recommend-monthprice">首付{{item.show_price}}万</span>
                    <div v-if="item.isAsk && item.enquiry_status == 1" class="pricedetail-div" @click="askPriceOne(index)">一键询价</div>
                    <div v-if="!item.isAsk && item.enquiry_status == 1" class="asked-price">已询价</div>
                  </div>
                </div>
              </div>
            </div>
    ...
    if (document.querySelector("#askPrice")) {
      document
        ...
        .addEventListener("touchmove", function(e) {
          if (e.target.className != "similar-recommend") {
            e.preventDefault();
          }
        });
      return;
...
.askprice-focus {
  font-size: 0.26rem;
  line-height: 0.26rem;
  border: 1px solid rgba(248, 93, 0, 1);
