<template>
  <div >
    <mip-fixed type="bottom" class="buttonWrap" v-if="source == 1">
        <!-- 非百城我要优惠为询底价 -->
        <button class="preButton button"  v-if="isBaiCheng==0"  @click='showAsk'>我要优惠</button>
        <!-- 百城我要优惠为IM 专家坐席-->
        <a class="preButton button" v-if="isBaiCheng ==1" :href="imUrl" @click="askClick">我要优惠</a>
        <!-- 电话组件，非百城拨打普通400电话  百城拨打天津400电话池-->
         <a ref='telPhone' :href="'tel:' + telPhone" ></a>
         <button class="callButton button"  @click="call">电话客服</button>
        <!-- 非百城 普通IM  百城 专家IM-->
        <a @click="gotoIm"  class="chatButton button" :href="imUrl">在线客服</a>
    </mip-fixed>
    <mip-fixed type="bottom">
        <a @click="gotoIm" class="chatButton button fixedButton" v-if="source==2" :href="imUrl">在线客服</a>
    </mip-fixed>
    <div v-if="source==3">
...
import { getLocalStorage,getCarId, getCityId } from "../../common/utils/utils.js";
