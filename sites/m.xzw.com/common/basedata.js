function range (anchor, n, m, polyfill = false, unit = '') {
  let arr = []
  let posIndex = ''
  for (let i = n, j = 0; i <= m; i++) {
    let value = (polyfill && i < 10 ? '0' + i : i) + unit
    arr.push(value)
    if (i + unit === anchor.value) {
      posIndex = j
    }
    j++
  }
  return {arr: arr, pos: posIndex}
}
function isLeapYear (y) {
  return (y % 4 === 0) && (y % 100 !== 0 || y % 400 === 0)
}
function getDays (anchor, y, m, UNIT_DAY) {
  y = Number(y)
  m = Number(m)
  let endDay = null
  switch (m) {
  ...
  const day = range(anchor, 1, endDay, false, UNIT_DAY)
  let days = {}
  days.arr = day.arr.map((day, idd) => {
    let id = idd + 1
    return {id: id, value: day}
  })
  days.pos = day.pos
  return days
}
export const f = function (type, posVal, region) {
  let _region = region || ''
  // 日期数据
  if (type === 'date' || type === 'date2' || type === 'time') {
    const START_YEAR = _region[0]
    const END_YEAR = _region[1]
    ...
    let anchor = [
      {value: `${currDate.getFullYear()}${UNIT_YEAR}`},
      {value: `${currDate.getMonth() + 1}${UNIT_MONTH}`},
      {value: `${currDate.getDate()}${UNIT_DAY}`},
      {value: `${currDate.getHours()}${UNIT_HOUR}`},
      {value: `${currDate.getMinutes()}${UNIT_MINUTE}`}
    ]
    if (posVal) {
      if (type === 'date2') {
        anchor[0] = posVal[0] ? {value: `${posVal[0]}${UNIT_YEAR}`} : anchor[0]
        anchor[1] = posVal[1] ? {value: `${posVal[1]}${UNIT_MONTH}`} : anchor[1]
      } else if (type === 'date') {
        anchor[0] = posVal[0] ? {value: `${posVal[0]}${UNIT_YEAR}`} : anchor[0]
        anchor[1] = posVal[1] ? {value: `${posVal[1]}${UNIT_MONTH}`} : anchor[1]
        anchor[2] = posVal[2] ? {value: `${posVal[2]}${UNIT_DAY}`} : anchor[2]
      } else if (type === 'time') {
        anchor[3] = posVal[3] ? {value: `${posVal[0]}${UNIT_HOUR}`} : anchor[3]
        anchor[4] = posVal[4] ? {value: `${posVal[1]}${UNIT_MINUTE}`} : anchor[4]
      }
    }
    const yearData = range(anchor[0], START_YEAR, END_YEAR, false, UNIT_YEAR)
    const monthData = range(anchor[1], 1, 12, false, UNIT_MONTH)

    const cascadeMonthData = monthData.arr.map((month) => {
      return {
        value: month,
        childs: []
      }
    ...
    let idx = 0
    let posDate = [yearData.pos, monthData.pos]
    const date = yearData.arr.map((year) => {
      idx++
      const item = {
        id: idx,
        value: year,
        childs: cascadeMonthData.slice()
      }
      item.childs.forEach((month, idm) => {
        let id = idm + 1
        month.id = id
        if (type !== 'date2') {
          let dayData = getDays(anchor[2], year.slice(0, -1), month.value.slice(0, -1), UNIT_DAY)
          month.childs = dayData.arr
          posDate[2] = dayData.pos
        }
      })
      return item
    })
    // 时间
    const hourData = range(anchor[3], 0, 23, true, UNIT_HOUR)
    const minuteData = range(anchor[4], 0, 59, true, UNIT_MINUTE)
    const time = [hourData.arr, minuteData.arr]
    const posTime = [hourData.pos, minuteData.pos]
    return type === 'time' ? {data: time, position: posTime} : {data: date, position: posDate}
  } else {
    // 地点
    const city = [{
      's': '\u5317\u4EAC',
    ...
    let place = []
    for (let c = 1; c < city.length; c++) {
      let cc = []
      for (let cs = 1; cs < city[c - 1].c.length; cs++) {
        cc.push({
          id: cs,
          value: city[c - 1].c[cs - 1]
        })
      }
      place.push({
        id: c,
