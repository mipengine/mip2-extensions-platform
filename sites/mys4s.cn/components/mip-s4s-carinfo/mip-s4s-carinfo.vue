    ...
    <div class="s4s-car-info s4s-illegal-body">
      <h2 class="s4s-car-name">或直接输入行驶证信息</h2>
        ...
        <div class="s4s-group" v-if="engineNoLength!== 0">
            ...
            <input type="text" :maxlength="engineNoLength === 99 ? '' : 6" v-model="engine" :placeholder="engineNoLength === 99 ? '请输入完整的发动机号' : '请输入发动机号后六位'" />
            <span class="s4s-help" @click="engineDetail">?</span>
        </div>
    </div>
    <button class="s4s-btn" @click="formSubmit"> {{isEdit?'保存':'添加'}} </button>
...
export default {
  ...
  methods: {
    ...
    classDetail() {
      this.detail = true;
      this.src = "https://s4s-imges.oss-cn-hangzhou.aliyuncs.com/img/classNo.png";
    },
    // 查看发动机号
    engineDetail() {
      this.detail = true;
    ...
    formSubmit() {
      ...
      if (this.engine.length < 6 && this.engineNoLength !== 0) {
       ...
       util.fetchData("v2/car/vio_manage", param).then(res => {
          if (res.code > 0) {
            util.toast(res.msg);
            return;
          }
