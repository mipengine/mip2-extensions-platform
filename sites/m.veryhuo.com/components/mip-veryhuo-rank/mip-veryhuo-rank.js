...
export default class MIPMhotComment extends CustomElement {
  build () {
    ...
    // 点击排行页的 箭头，实现显示隐藏功能
    let zk = this.element.querySelector('.zk');   // 下箭头
    let sq = this.element.querySelector('.sq');   // 上箭头
    let rankA = this.element.querySelectorAll('.topnav a');   // 所有标题
    ...
    // 机型判断
    let u = navigator.userAgent, app = navigator.appVersion;
    let isIphone= !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/)  //ios终端
    var iosweb = document.getElementById('iosweb').getAttribute('value');
    var classweb = document.getElementById('classweb').getAttribute('value');
    var isyouxi = document.getElementById('isyouxi').getAttribute('value');
    var typeid = document.getElementById('typeid').getAttribute('value');
